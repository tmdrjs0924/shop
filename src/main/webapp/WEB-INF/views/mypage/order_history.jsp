...
<title>Insert title here</title>
	...
	<script>
		...
		function review_popup(pCode,pImg1,pName,pSize,index,id){
			var popX=(window.screen.width/2)-(450/2);
			var popY=(window.screen.height/2)-420;
			window.open("review?pCode="+pCode+"&pImg1="+pImg1+"&pName="+pName+"&pSize="+pSize+"&index="+index+"&id="+id,"","width=450,height=420,left="+popX+",top="+popY);
		}
	</script>
</head>
<body>
				...
				<tr class="content">
					...
					<td class="right">
						<c:if test="${ovo.orderState==10 && pvo.rchk==0}">
							<div onclick="review_popup('${pvo.pCode}','${pvo.pImg1}','${pvo.pName}','${pvo.pSize}',${sts.index},${ovo.id})">구매후기</div>
						</c:if>
